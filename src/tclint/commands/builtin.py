...
def _proc_1(name, args, parser):
    if len(args) != 3:
        raise CommandArgError(f"wrong # of args to {name}: got {len(args)}, expected 3")
...
def _proc(args, parser):
    return _proc_1("proc", args, parser)
...
def _gdb_caching_proc(args, parser):
    return _proc_1("gdb_caching_proc", args, parser)


def _proc_with_prefix(args, parser):
    return _proc_1("proc_with_prefix", args, parser)
...
gdb_testsuite_commands = commands_schema({
